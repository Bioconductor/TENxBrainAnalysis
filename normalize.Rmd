...
library(scater)
sce <- as(readRDS("objects/qc_mat.rds"), "SingleCellExperiment")
```
...
This is done by applying the `quickCluster` function on the cells generated from each library.
...
for (lib in names(by.lib)) { 
    current <- by.lib[[lib]]
    ids <- quickCluster(sce[,current], subset.row=keep, 
                        method="igraph", max.size=3000)
...
```{r}
library(scran)
sce <- computeSumFactors(sce, cluster=mod.source, subset.row=keep)
...
```{r sizefacplot}
plot(sce$total_counts, sfs, log="xy", xlab="Library size", 
