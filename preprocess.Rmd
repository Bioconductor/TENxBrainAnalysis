...
We start with the log-library size:

```{r libsize}
plotColData(sce, c("scater_qc", "all", "log10_total_counts"), x="Library")
```

... and then the log-number of detected features per cell:

```{r numfeat}
plotColData(sce, c("scater_qc", "all", "log10_total_features_by_counts"), x="Library")
```

... and finally the proportion of mitochondrial counts:

```{r mitoprop} 
plotColData(sce, c("scater_qc", "feature_control_Mito", "pct_counts"), x="Library")
```
...
For further diagnostics, we calculate the average count, adjusted for library size.
We re-use the total counts as the size factors to avoid computing them again.

```{r ave-per-gene}
system.time({
    ave.count <- calcAverage(sce, BPPARAM=BPPARAM, 
        use_size_factors=sce$total_counts)
})
rowData(sce)$AverageCount <- ave.count

pct.cells <- rowData(sce)$scater_qc$all$n_cells_by_counts/ncol(sce) * 100
smoothScatter(log10(ave.cout), pct.cells, ylab="Percentage of cells",
...
```{r}
cols <- c("Ensembl", "Symbol", "AverageCount")
top.genes <- rowData(sce)[,cols]
